...
public class ProviderMicroService {
	...
	public Response addProvider(ProviderDto dto) {
		...
		try {
			...
			URI location = uriInfo.getBaseUriBuilder().path(provider.getId().toString()).build();
			return Response.created(location).build();
		} catch (Exception e) {
			logger.fatal("Failed to add provider!", e);
	...
	public Response addTreatment(@PathParam("id") String id, TreatmentDto dto) {
		try {
			...
			logger.fatalf(e, "Failed to add treatment %s!", id);
			return Response.serverError().build();
		}
	}
	...
	private UUID addTreatmentImpl(TreatmentDto dto, Consumer<Treatment> parentFollowUps)
		...
		if (dto instanceof DrugTreatmentDto drugTreatmentDto) {
		...
		} else if (dto instanceof PhysiotherapyTreatmentDto physioTreatmentDto) {
			...
			followUpsConsumer = provider.importPhysiotherapy(
					...
					provider,
					dto.getDiagnosis(),
					physioTreatmentDto.getTreatmentDates(),
					parentFollowUps);
		}
	...
	public TreatmentDto getTreatment(@PathParam("id") String id, @PathParam("tid") String tid) {
		try {
			...
			logger.infof("Failed to find provider with id %d", id);
			throw new WebApplicationException(Status.NOT_FOUND);
		} catch (TreatmentExn e) {
			logger.infof("Failed to find treatment %s for provider %s", tid, id);
			throw new WebApplicationException(Status.NOT_FOUND);
		} catch (Exception e) {
